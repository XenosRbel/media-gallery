...
const MediaFolder = ({ filesPath, folderId, files, folderSystemName }) => {
  ...
  const handleFirstItemDefaultGroup = (itemIndex, fileId) => {
    if (
      folderSystemName === DEFAULT_SYSTEM_GROUP_NAME &&
  ...
  return (
    <Row key={`folder-${folderId}`} gutter={ROW_GUTTER}>
      ...
      <Col key="upload-form" md={4} className="center">
        <Upload
          ...
          customRequest={(params) => handleSubmitFile(params, filesPath, folderId)}
...
MediaFolder.propTypes = {
  ...
  folderSystemName: PropTypes.string,
};

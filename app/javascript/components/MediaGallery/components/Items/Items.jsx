  ...
  <Collapse defaultActiveKey={DEFAULT_ACTIVE_ACCORDION_KEY}>
    {records.map(({ id, attributes: { name, systemName, files }, links }) => (
      <Collapse.Panel
        ...
        key={id}
        extra={shouldShowGroupActions && <DropDownItemActions links={links} groupAttr={{ name, systemName }} />}
      >
        {cloneElement(galleryComponent, { files, groupId: id, groupSystemName: systemName })}
      </Collapse.Panel>
    ))}
  </Collapse>
);
